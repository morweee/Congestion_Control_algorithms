...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
    ...
    transitTimeRecord= [0,0]
    transitTimeDelayRecord = []
    transitTimeGreaterThanAverage = 0
    while seq_id < len(data):
        print(f"WindowSize:{CWND}")
        ...
        while True:
            strat_recvPacket = time.time()
            try:
                # wait for ack and extract ack id
                ack, _ = udp_socket.recvfrom(PACKET_SIZE)
                ack_id = int.from_bytes(ack[:SEQ_ID_SIZE], byteorder='big')
                if transitTimeGreaterThanAverage <50:
                    transitTime = time.time() - strat_recvPacket
                    transitTimeRecord[0] += transitTime
                    transitTimeRecord[1] += 1
                    transitTimeDelayRecord.append(transitTime)
                    if transitTime < transitTimeRecord[0]/transitTimeRecord[1]:
                        transitTimeGreaterThanAverage+=1
                        if transitTimeGreaterThanAverage >= 50:
                            CWND = round(1.1/(sum(sorted(transitTimeDelayRecord)[len(transitTimeDelayRecord)//2:])/(len(transitTimeDelayRecord)//2)))
                            ssthresh = CWND
                    else:
                        transitTimeGreaterThanAverage = 0
    ...
    print(sum(sorted(transitTimeDelayRecord)[len(transitTimeDelayRecord)//2:])/(len(transitTimeDelayRecord)//2))
